...
def predict_speed(
    ...
    """if len(bottom_position_of_detected_vehicle) != 0 and bottom - bottom_position_of_detected_vehicle[0] > 0 and 205 \
        < bottom_position_of_detected_vehicle[0] and bottom_position_of_detected_vehicle[0] < 210 \
        and roi_position < bottom:
        is_vehicle_detected.insert(0, 1)
        update_csv = True
        image_saver.save_image(crop_img)  # save detected vehicle image
    """
    if len(bottom_position_of_detected_vehicle) != 0 and bottom - bottom_position_of_detected_vehicle[0] > 0 and \
            bottom_position_of_detected_vehicle[0] < 210 and \
            roi_position < bottom:
        is_vehicle_detected.insert(0, 1)
        update_csv = True
        image_saver.save_image(crop_img)  # save detected vehicle image
    ...
    #print(len(bottom_position_of_detected_vehicle) != 0, bottom - bottom_position_of_detected_vehicle[0] > 0, 205 \
    #    < bottom_position_of_detected_vehicle[0], bottom_position_of_detected_vehicle[0] < 210 \
    #    , roi_position < bottom)
    # for debugging
    ...
    return (direction, sum(Allspeed) / len(Allspeed), is_vehicle_detected, update_csv)
